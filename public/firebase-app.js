...
function setPrintPageSize(size) {
    // La CSS de impresión forzada también es compartida (albaran_render.js).
    return window.npSetPrintPageSize(size);
}
...
// ── Formato del albarán: véase albaran_render.js ──────────────────────
// Ese fichero es la FUENTE ÚNICA del formato físico y lo cargan también el
// admin, el conductor y la previsualización: así el albarán sale igual se
// imprima desde donde se imprima. app.html lo carga ANTES que este script.
const NOVAPACK_CARRIER   = window.NOVAPACK_CARRIER;
const npBultosExpandidos = window.npBultosExpandidos;
const npTotalBultos      = window.npTotalBultos;
const npQrField          = window.npQrField;
