                ...
                async function downloadFullBackupJSON() {
                    if (!confirm("Â¿Desea descargar una copia de seguridad local de toda su base de datos?")) return;
                    showLoading();
                    try {
                        ...
                        alert("âœ… Copia de seguridad descargada correctamente.");
                    } catch (e) {
                        console.error("Backup Error:", e);
                        alert("âŒ Error al generar la copia de seguridad: " + e.message);
                    } finally {
                        hideLoading();
                    }
                }
            ...
            window.logout = () => {
                if (confirm("Â¿Cerrar sesiÃ³n en Novapack Cloud?")) {
            ...
            async function initArticlesView() {
                ...
                tbody.innerHTML = '<tr><td colspan="4">Cargando catÃ¡logo...</td></tr>';
                try {
            ...
            function renderArticlesTable(articles) {
                ...
                articles.forEach(data => {
                    ...
                    <td style="color:var(--brand-primary); font-weight:bold;">${data.basePrice ? data.basePrice.toFixed(2) + 'â‚¬' : '-'}</td>
                    <td>
                        <div style="display:flex; gap:5px;">
                            <button class="btn btn-outline btn-sm" onclick="editArticle('${data.id}')">âœï¸</button>
                            <button class="btn btn-outline btn-sm" onclick="deleteArticle('${data.id}')">ðŸ—‘ï¸</button>
                        </div>
                    </td>
                `;
            ...
            window.openNewArticleModal = () => {
                document.getElementById('article-modal-title').textContent = "AÃ±adir ArtÃ­culo Master";
            ...
            document.getElementById('article-form').onsubmit = async (e) => {
                ...
                try {
                    ...
                    initArticlesView();
                } catch (e) { alert("Error al guardar artÃ­culo."); }
            };

            window.deleteArticle = async (id) => {
                if (confirm("Â¿Eliminar este artÃ­culo del catÃ¡logo maestro?")) {
            ...
            window.addAbonoLine = () => {
                ...
                if (isNaN(price) || price <= 0) { alert("Ingrese un importe base vÃ¡lido."); return; }
            ...
            function renderAbonoLines() {
                ...
                currentAbonoLines.forEach((l, idx) => {
                    ...
                    row.innerHTML = `
                    <td>${l.name}</td>
                    <td style="color:#ff4444">- ${l.base.toFixed(2)}â‚¬</td>
                    <td>${l.iva}%</td>
                    <td style="font-weight:bold; color:#ff4444">- ${l.total.toFixed(2)}â‚¬</td>
                    <td><button class="btn btn-outline btn-sm" onclick="removeAbonoLine(${idx})">âŒ</button></td>
                `;
                    tbody.appendChild(row);
                });
            }
            ...
            window.saveAbonoTicket = async () => {
                ...
                if (currentAbonoLines.length === 0) { alert("AÃ±ada lÃ­neas al abono para continuar."); return; }

                if (!confirm("Â¿Desea generar esta factura de abono? Los importes se guardarÃ¡n en negativo.")) return;
                ...
                try {
                    ...
                    alert("âœ… Factura de Abono Generada con Ã©xito.");
                    showView('billing');
            ...
            async function initCreditNotesListView() {
                ...
                try {
                    ...
                    tbody.innerHTML = '';
                    if (snap.empty) {
                        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:20px; color:#888;">No se han emitido facturas de abono todavÃ­a.</td></tr>';
                        return;
                    }
                    snap.forEach(doc => {
                        const data = doc.data();
                        ...
                        <td style="color:#ff4d4d; font-weight:bold;">${data.total ? data.total.toFixed(2) : '0.00'}â‚¬</td>
                        <td>
                             <button class="btn btn-outline btn-sm" onclick="printTicketFromAdmin('${data.uid}', '${data.compId}', '${doc.id}')" title="Imprimir Comprobante">ðŸ–¨ï¸</button>
                        </td>
                    `;
                        tbody.appendChild(row);
                    });
            ...
            async function _doWipeOperationalData() {
                ...
                // 1. Borrar todos los albaranes (Tickets) - ColecciÃ³n Global
                const ticketsSnap = await db.collection('tickets').get();
                ...
                for (const doc of ticketsSnap.docs) {
                    ...
                    count++;
                    if (count >= 400) { // LÃ­mite de batch de Firebase (500)
                        await batch.commit();
                        batch = db.batch();
                        count = 0;
                    }
            ...
            // FUNCIÃ“N DE RESET TOTAL DE DATOS OPERATIVOS
            window.wipeAllOperationalData = async () => {
                const p1 = confirm("âš ï¸ ATENCIÃ“N: Vas a borrar TODOS los albaranes y facturas de la base de datos.\n\nÂ¿EstÃ¡s completamente seguro?");
                if (!p1) return;

                const p2 = confirm("ESTA ACCIÃ“N NO SE PUEDE DESHACER.\nLos contadores de facturas y albaranes volverÃ¡n a empezar desde el nÃºmero 1.\n\nÂ¿Deseas continuar?");
                if (!p2) return;

                const code = prompt("Escribe 'BORRAR TODO' para confirmar definitivamente:");
                if (code !== 'BORRAR TODO') {
                    alert("ConfirmaciÃ³n incorrecta. AcciÃ³n cancelada.");
                    return;
                }
                ...
                try {
                    ...
                    alert("âœ… Datos operativos borrados. Los contadores han vuelto a 1.");
                } catch (e) {
                    console.error(e);
            ...
            window.fullAppReset = async () => {
                const p1 = confirm("ðŸš¨ RESET TOTAL DE LA APLICACIÃ“N ðŸš¨\n\nEsto borrarÃ¡:\n- TODOS los Clientes (excepto tÃº)\n- TODAS las Tarifas\n- TODOS los Albaranes y Facturas\n- TODA la configuraciÃ³n\n\nÂ¿EstÃ¡s SEGURO?");
                if (!p1) return;
            ...
            // FunciÃ³n para reparar la base de datos (Admin Privs & Indexes)
            window.repairDatabase = async () => {
                if (!confirm("Esta acciÃ³n reconstruirÃ¡ los privilegios de administrador y verificarÃ¡ la estructura base. Â¿Deseas continuar?")) return;
                showLoading();
                try {
                    if (auth.currentUser) {
                        // Restaurar doc de admin explÃ­citamente
                    ...
                    await loadUsers();
                    alert("âœ… ReparaciÃ³n completada e Ã­ndices refrescados.");
                } catch (e) {
                    console.error("Repair failed:", e);
                    alert("Error en reparaciÃ³n: " + e.message);
                } finally {
                    hideLoading();
                }
            };
            ...
            window.reindexUserIds = async () => {
                if (!confirm("âš ï¸ Â¿RE-INDEXAR IDs DE CLIENTE?\n\nEsto asignarÃ¡ nÃºmeros correlativos (101, 102...) a todos los clientes basados en su fecha de creaciÃ³n si les falta el ID o hay duplicados.\n\nÂ¿Deseas continuar?")) return;
                showLoading();
                try {
                    ...
                    let nextId = 101;
                    snap.forEach(doc => {
                        batch.update(doc.ref, { idNum: String(nextId) });
                        nextId++;
                    });
                    await batch.commit();
                    alert(`âœ… Se han re-indexado ${snap.size} usuarios.`);
                    loadUsers();
                } catch (e) {
            ...
            window.syncAllTicketsIndices = async () => {
                const confirmed = confirm("ðŸ” SINCRO GLOBAL DE ALBARANES\n\nEste proceso buscarÃ¡ albaranes que no aparecen en la App del cliente (por IDs antiguos o inconsistentes) y los vincularÃ¡ al ID actual de cada usuario.\n\nÂ¿Ejecutar ahora?");
                if (!confirmed) return;
            ...
            async function fetchAllUsersMap() {
                ...
                try {
                    // Escucha incremental para eficiencia en grandes volÃºmenes
            ...
            async function loadUsers(direction = 'first') {
                ...
                try {
                    ...
                    // pero createdAt es el estÃ¡ndar del sistema.
                    let query = db.collection('users').orderBy('name', 'asc'); // Cambiar a 'name' para que sea mÃ¡s estable si faltan fechas
                    ...
                    if (snapshot.empty) {
                        userTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:30px; color:#888;">No hay clientes registrados en esta pÃ¡gina.</td></tr>';
                        ...
                        snapshot.forEach(doc => {
                            ...
                            <td>
                                <div style="display:flex; gap:5px;">
                                    <button class="btn btn-primary btn-sm" onclick="openEditUserModal('${data.id}')" title="Editar Cliente">âœï¸</button>
                                    <button class="btn btn-outline btn-sm" onclick="openManageCompaniesModal('${data.id}')" title="Sedes/Empresas">ðŸ¢</button>
                                    <button class="btn btn-outline btn-sm" onclick="openManageDestinationsModal('${data.id}')" title="Agenda/Direcciones">ðŸ“‡</button>
                                    <button class="btn btn-secondary btn-sm" onclick="sendWhatsApp('${data.senderPhone || ''}', 'Hola ${data.name}')" title="WhatsApp">ðŸ’¬</button>
                                    <button class="btn btn-danger btn-sm" onclick="deleteUser('${data.id}')" title="Eliminar">ðŸ—‘ï¸</button>
                                </div>
                            </td>
                        `;
                        ...
                        document.getElementById('label-users-page').textContent = `PÃ¡gina ${usersPage}`;
            ...
            addUserForm.onsubmit = async (e) => {
                ...
                // Construir direcciÃ³n para compatibilidad
                const parts = [];
                if (street) parts.push(street);
                if (num) parts.push("NÂº " + num);
                ...
                try {
                    ...
                    // 1. VERIFICACIÃ“N DE DUPLICADOS (Seguridad e Independencia)
                    if (!userId) {
                        // Si es nuevo cliente, probamos si el email ya existe
                        const docEmail = await db.collection('users').doc(email).get();
                        if (docEmail.exists) {
                            alert(`âŒ ERROR: El email "${email}" ya estÃ¡ registrado para otro cliente.`);
                            hideLoading();
                            return;
                        }

                        // Probamos si el ID de cliente ya existe (como string o como nÃºmero)
                        ...
                        if (!snapId.empty || !snapIdNum.empty) {
                            ...
                            alert(`âš ï¸ AVISO: El ID #${idNum} ya pertenece a "${existing.name}".\nPor favor, usa un ID Ãºnico.`);
                            hideLoading();
                            return;
                        }
                    }
                    ...
                    if (userId) {
                        ...
                        // CreaciÃ³n de nuevo registro maestro
                    ...
                    const mainCompData = {
                        name: name,
                        address: senderAddress || 'DirecciÃ³n no definida',
                        street: street || '',
                        number: num || '',
                        localidad: city || '',
                        cp: cp || '',
                    ...
                    loadUsers();
                    alert(userId ? "âœ… Cliente actualizado correctamente." : "âœ… Cliente creado con Ã©xito en la nube.");
                } catch (err) {
                    console.error(err);
            ...
            async function initReportsView() {
                ...
                try {
                    ...
                    // Fallback a collectionGroup si la anterior falla por alguna razÃ³n de permisos
            ...
            document.getElementById('btn-search-reports').onclick = async () => {
                ...
                status.textContent = "Buscando envÃ­os y aplicando tarifas...";
                ...
                try {
                    ...
                    snap.forEach(doc => {
                        ...
                        // IdentificaciÃ³n del propietario (Ahora por campos explicitos, no por ruta)
                    ...
                    if (reportsData.length === 0) {
                        ...
                        reportsData.sort((a, b) => {
                            ...
                            <td style="font-weight:bold; color:#FF6600;">${t.cod ? t.cod + 'â‚¬' : '-'}</td>
                            <td>
                                <div style="display:flex; gap:5px;">
                                    <button class="btn btn-outline btn-sm" onclick="printTicketFromAdmin('${t.uid}', '${t.compId}', '${t.docId}')" title="Imprimir">ðŸ–¨ï¸</button>
                                    <button class="btn btn-secondary btn-sm" onclick="sendWhatsAppTicket('${t.uid}', '${t.compId}', '${t.docId}')" title="WhatsApp">ðŸ’¬</button>
                                </div>
                            </td>
        `;
                            tbody.appendChild(row);
    ...
    <td>${data.number}</td>
    <td><button class="btn btn-danger btn-sm" onclick="deletePhone('${doc.id}')">ðŸ—‘ï¸</button></td>`;
            ...
            btnAddUser.onclick = () => {
                document.getElementById('modal-user-title').textContent = "AÃ±adir Nuevo Cliente";
                document.getElementById('edit-user-uid').value = "";
                addUserForm.reset();
                // Auto-generar el siguiente ID lÃ³gico
            ...
            function renderSubTariffTabs() {
                ...
                btnMain.textContent = "ESTÃNDAR (PRINCIPAL)";
            ...
            function renderTariffLevel() {
                ...
                if (keys.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="3" style="text-align:center; color:#888;">No hay artÃ­culos.</td></tr>';
                    return;
                }
                keys.forEach(name => {
                    const row = document.createElement('tr');
                    row.innerHTML = `<td style = "font-weight:700" > ${name}</td><td>${parseFloat(items[name]).toFixed(2)}â‚¬</td>
                    <td style="text-align:right"><button class="btn btn-danger btn-sm" onclick="deleteTariffItem('${name}')">ðŸ—‘ï¸</button></td>`;
                    tbody.appendChild(row);
                });
            }
            ...
            window.deleteCurrentSubTariff = async () => {
                ...
                if (!confirm(`Â¿Eliminar permanentemente la sub - tarifa ${activeSubTariff}?`)) return;
            ...
            window.deleteAdminDest = async (docId) => {
                if (!confirm("Â¿Eliminar esta entrada de la agenda?")) return;
                showLoading();
                try {
                    const uid = adminTicketUID;
                    const uData = userMap[uid] || {};
                    const targetUid = uData.authUid || uData.id || uid;
                    if (!targetUid || targetUid === "") throw new Error("ID de cliente no vÃ¡lido.");
            ...
            window.editCompany = (id, name, prefix, street, num, city, cp, province, phone, subT, startNum) => {
                document.getElementById('edit-company-id').value = id;
                document.getElementById('comp-name').value = name;
                document.getElementById('comp-prefix').value = prefix || '';
                document.getElementById('comp-start-num').value = startNum || '1001';
                document.getElementById('comp-street').value = street || '';
                document.getElementById('comp-number').value = num || '';
                document.getElementById('comp-city').value = city || '';
                document.getElementById('comp-cp').value = cp || '';
                document.getElementById('comp-province').value = province || '';
                document.getElementById('comp-phone').value = phone || '';
                document.getElementById('comp-subtariff-id').value = subT || '';
            ...
            async function loadInvoiceCompanies(uid) {
                ...
                try {
                    ...
                    if (!snap || snap.empty) {
                        console.warn("[Billing] No se encontraron empresas para este cliente. AÃ±adiendo opciÃ³n virtual de Empresa Principal.");
                        ...
                        select.innerHTML = '<option value="comp_main">ðŸ  Empresa Principal (AutomÃ¡tica)</option>';
                        select.value = "comp_main";
                        refreshUnbilledList();
                    } else {
                        console.log(`[Billing] Ã‰xito.Usando UID: ${workingUid}.Encontradas: ${snap.size} `);
                    ...
                    console.error("Error crÃ­tico cargando empresas:", e);
            ...
            async function refreshUnbilledList() {
                ...
                try {
                    ...
                    // Intentamos buscar por clientIdNum que es lo mÃ¡s consistente
                    ...
                    snap.forEach(doc => {
                        ...
                        // VALIDACIÃ“N MULTI-FACTOR PERFECCIONADA
                        ...
                        // 2. FILTRAR POR ESTADO DE FACTURACIÃ“N
                        if (t.invoiceId && String(t.invoiceId).trim() !== "" && String(t.invoiceId).toLowerCase() !== "null") {
                            filteredByBilled++;
                            return;
                        }

                        // 3. FILTRAR POR EMPRESA (LÃ³gica inteligente para 'Empresa Principal')
                        ...
                        if (compId && compId !== "ALL") {
                            if (isMainCompSelected) {
                                ...
                                // Si seleccionamos una especÃ­fica secundarÃ­a, debe coincidir exactamente
                    ...
                    if (tickets.length === 0) {
                        ...
                        if (matchCount === 0) {
                            ...
                            detail = `${filteredByBilled} ya estÃ¡n facturados, ${filteredByDate} fuera de fecha o empresa seleccionada diferente.`;
                        }
                    ...
                    console.error("Error crÃ­tico en Billing Sync (v2.4):", e);
                    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:red; padding:20px;">
                    <strong>Error de sincronizaciÃ³n (v2.4)</strong><br>
        ...
        `;
            ...
            window.updateInvoiceCalculations = () => {
                ...
                document.getElementById('inv-calc-subtotal').textContent = subtotal.toFixed(2) + 'â‚¬';
                document.getElementById('inv-calc-iva').textContent = iva.toFixed(2) + 'â‚¬';
                document.getElementById('inv-calc-irpf').textContent = irpf.toFixed(2) + 'â‚¬';
                document.getElementById('inv-calc-total').textContent = total.toFixed(2) + 'â‚¬';
            ...
            document.getElementById('btn-create-invoice-final').onclick = async () => {
                ...
                try {
                    ...
                    alert(`Factura ${invoiceData.invoiceId} generada con Ã©xito.`);
            ...
            async function loadInvoices(direction = 'first') {
                ...
                try {
                    ...
                    if (snap.empty) {
                        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; color:#888;">No hay mÃ¡s facturas.</td></tr>';
                        ...
                        snap.forEach(doc => {
                            ...
                            <td>${inv.subtotal.toFixed(2)}â‚¬</td>
                            <td>${inv.iva.toFixed(2)}â‚¬ (${inv.ivaRate}%)</td>
                            <td>${inv.irpf.toFixed(2)}â‚¬ (${inv.irpfRate}%)</td>
                            <td style="font-weight:900; color:var(--brand-primary)">${inv.total.toFixed(2)}â‚¬</td>
                            <td>
                                <div style="display:flex; gap:5px;">
                                    <button class="btn btn-outline btn-sm" onclick="printInvoice('${doc.id}')">ðŸ–¨ï¸</button>
                                    <button class="btn btn-secondary btn-sm" onclick="emailInvoice('${doc.id}')">ðŸ“§</button>
                                    <button class="btn btn-secondary btn-sm" style="background:#25D366; border-color:#25D366;" onclick="sendWhatsAppInvoice('${doc.id}')">ðŸ’¬</button>
                                    <button class="btn btn-outline btn-sm" style="color:#FF4444;" onclick="downloadInvoicePDF('${doc.id}')">ðŸ“„</button>
                                    <button class="btn btn-danger btn-sm" onclick="deleteInvoice('${doc.id}')">ðŸ—‘ï¸</button>
                                </div>
                            </td>
                        `;
                            tbody.appendChild(tr);
                        ...
                        document.getElementById('label-invoices-page').textContent = `PÃ¡gina ${invoicesPage}`;
            ...
            document.getElementById('btn-generate-sepa').onclick = async () => {
                ...
                try {
                    ...
                    alert("Remesa SEPA generada correctamente. SÃºbala ahora a la web de su banco.");
                } catch (e) { alert("Error: " + e.message); }
                finally { hideLoading(); }
            };
            ...
            function renderTaxSummary(s303, s111) {
                ...
                <strong>${q}</strong><br>
                Base Imponible: ${data.base.toFixed(2)}â‚¬ |
                <span style="color:#4CAF50">IVA Devengado: ${data.iva.toFixed(2)}â‚¬</span>
            </div>
        `;
                });
            ...
            <div style = "background:rgba(255,255,255,0.03); padding:10px; border-radius:4px; margin-bottom:10px;" >
                <strong>${q}</strong><br>
                Base Retenciones: ${data.base.toFixed(2)}â‚¬ |
                <span style="color:#FF3B30">IRPF a Ingresar: ${data.irpf.toFixed(2)}â‚¬</span>
            </div>
        `;
                });
            }
            ...
            <body style="background:#f5f5f5; padding: 20px;">
                <div id="inv-content" style="max-width: 800px; margin: 0 auto; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                    ...
                    <div class="no-print" style="text-align:center; padding: 20px;">
                        <button onclick="window.print()" style="padding:15px 30px; background:#FF6600; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">ðŸ–¨ï¸ IMPRIMIR FACTURA</button>
                    </div>
                </div>
            </body>
            </html>
            ...
            window.emailInvoice = async (id) => {
                ...
                const subject = encodeURIComponent(`Factura ${inv.invoiceId} - NOVAPACK LOGÃSTICA`);
                const body = encodeURIComponent(`Hola ${inv.clientName}, \n\nLe adjuntamos el detalle de su factura ${inv.invoiceId} por importe de ${inv.total.toFixed(2)}â‚¬.\n\nForma de pago: Transferencia a ${inv.senderData ? inv.senderData.bank : 'la cuenta habitual'} \n\nGracias por confiar en NOVAPACK.`);
            ...
            window.sendWhatsApp = (phone, msg) => {
                if (!phone) { alert("Sin telÃ©fono configurado."); return; }
            ...
            window.sendWhatsAppInvoice = async (id) => {
                ...
                const msg = `Hola ${inv.clientName}, le enviamos su factura ${inv.invoiceId} (${inv.total.toFixed(2)}â‚¬). Saludos, NOVAPACK.`;
                window.sendWhatsApp(phone, msg);
            };
            ...
            window.sendWhatsAppTicket = async (uid, compId, docId) => {
                ...
                const msg = `Hola, consulta sobre el albarÃ¡n ${t.id} con destino ${t.receiver}.`;
                window.sendWhatsApp(client ? client.senderPhone : '', msg);
            };

            window.createCreditInvoice = async (id) => {
                if (!confirm("Â¿Desea crear una factura de abono (rectificativa) para esta factura? El importe serÃ¡ negativo.")) return;
                showLoading();
                try {
                    const doc = await db.collection('invoices').doc(id).get();
                    const inv = doc.data();
                    ...
                    alert(`Factura de abono ${creditId} generada con Ã©xito.`);
                    loadInvoices();
            ...
            window.printTicketFromAdmin = async (uid, compId, docId) => {
                ...
                // Marcar como impreso en la nube para sincronizaciÃ³n
                ...
                const win = window.open('', '_blank');
                win.document.write(`<html><head><meta charset="UTF-8"><title>AlbarÃ¡n ${t.id}</title><style>
                ...
                <div class="ticket-container">
                    <div class="header">
                        <div>
                            ...
                            <div style="color:#666; font-weight:600; margin-top:5px; font-size:0.9rem;">LOGÃSTICA Y DISTRIBUCIÃ“N</div>
                        </div>
                        <div class="id-box">
                            <div class="id-label">AlbarÃ¡n Oficial</div>
                    ...
                    <div class="details-box">
                        <h3 class="info-title">Especificaciones del EnvÃ­o</h3>
                        ...
                        <div class="detail-item">
                            ...
                            <span class="value">${t.timeSlot || 'MAÃ‘ANA'}</span>
                        </div>
                        <div class="detail-item">
            ...
            function generateInvoiceHTML(inv, date) {
                ...
                if (inv.ticketsDetail && inv.ticketsDetail.length > 0) {
                    ...
                    <tr>
                        <td style="padding: 15px; border-bottom: 1px solid #EEE;">
                            <div style="font-weight: bold; color: #333;">DELEGACIÃ“N: ${group}</div>
                            <div style="font-size: 0.85rem; color: #666; margin-top: 4px;">Albaranes: ${grouped[group].ids.join(', ')}</div>
                        </td>
                        <td style="padding: 15px; border-bottom: 1px solid #EEE; text-align: right;">${grouped[group].subtotal.toFixed(2)}â‚¬</td>
                    </tr>`;
                    });
                } else {
                    conceptHTML = `
                <tr>
                    ...
                    <td style="padding: 15px; border-bottom: 1px solid #EEE; text-align: right;">${inv.subtotal.toFixed(2)}â‚¬</td>
                </tr>`;
                }
            ...
            <div style="font-family: sans-serif; padding: 40px; color: #333; line-height: 1.6; background:white;">
                <div style="display: flex; justify-content: space-between; border-bottom: 2px solid #EEE; padding-bottom: 20px; margin-bottom: 40px;">
                    <div><div style="font-size: 2rem; font-weight: bold; color: #FF6600;">NOVAPACK LOGÃSTICA</div><div style="font-size: 1.2rem; color: #666;">FACTURA: ${inv.invoiceId}</div></div>
                ...
                <div style="float: right; width: 300px;">
                    <div style="display: flex; justify-content: space-between; padding: 5px 0;"><span>Base Imponible:</span><span>${inv.subtotal.toFixed(2)}â‚¬</span></div>
                    <div style="display: flex; justify-content: space-between; padding: 5px 0;"><span>IVA (${inv.ivaRate}%):</span><span>${inv.iva.toFixed(2)}â‚¬</span></div>
                    ${inv.irpf > 0 ? `<div style="display: flex; justify-content: space-between; padding: 5px 0;"><span>IRPF (-${inv.irpfRate}%):</span><span>-${inv.irpf.toFixed(2)}â‚¬</span></div>` : ''}
                    <div style="display: flex; justify-content: space-between; padding: 10px 0; font-size: 1.4rem; font-weight: bold; border-top: 2px solid #FF6600; color: #FF6600;"><span>TOTAL:</span><span>${inv.total.toFixed(2)}â‚¬</span></div>
                </div>
            ...
            window.deleteInvoice = async (id) => {
                if (!confirm("Â¿Eliminar factura y marcar albaranes como No Facturados?")) return;
                showLoading();
                try {
                    const doc = await db.collection('invoices').doc(id).get();
                    const inv = doc.data();
            ...
            async function initTaxModelsView() {
                ...
                try {
                    ...
                    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:10px; border-bottom:1px solid #333; padding:10px 0;">
                        ...
                        <span style="text-align:right; font-weight:bold;">${totalBase.toFixed(2)}â‚¬</span>
                    </div>
                    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:10px; border-bottom:1px solid #333; padding:10px 0;">
                        <span>IVA Repercutido (21%):</span>
                        <span style="text-align:right; font-weight:bold; color:#4CAF50;">+ ${totalIva.toFixed(2)}â‚¬</span>
                    </div>
                    <div style="margin-top:15px; text-align:right; font-size:1.1rem; color:#4CAF50;">
                        IVA A LIQUIDAR: <strong>${totalIva.toFixed(2)}â‚¬</strong>
                    </div>
                `;

                    document.getElementById('full-summary-111').innerHTML = `
                    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:10px; border-bottom:1px solid #333; padding:10px 0;">
                        <span>NÃºmero de perceptores (Facturas con IRPF):</span>
                    ...
                    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:10px; border-bottom:1px solid #333; padding:10px 0;">
                        ...
                        <span style="text-align:right; font-weight:bold;">${filteredInvoices.reduce((s, i) => s + (i.irpf > 0 ? i.subtotal : 0), 0).toFixed(2)}â‚¬</span>
                    </div>
                    <div style="display:grid; grid-template-columns: 2fr 1fr; gap:10px; border-bottom:1px solid #333; padding:10px 0;">
                        <span>Retenciones practicadas (IRPF):</span>
                        <span style="text-align:right; font-weight:bold; color:#FF9800;">${totalIrpf.toFixed(2)}â‚¬</span>
                    </div>
                    <div style="margin-top:15px; text-align:right; font-size:1.1rem; color:#FF9800;">
                        TOTAL A INGRESAR: <strong>${totalIrpf.toFixed(2)}â‚¬</strong>
                    </div>
                `;
            ...
            window.sendTaxesByEmail = () => {
                ...
                const body = encodeURIComponent(`Hola,\n\nAdjunto resumen tributario de NOVAPACK para el periodo ${period}T de ${year}:\n\n` +
                    `- Base Imponible: ${currentTaxData.base.toFixed(2)}â‚¬\n` +
                    `- IVA Repercutido: ${currentTaxData.iva.toFixed(2)}â‚¬\n` +
                    `- Retenciones IRPF: ${currentTaxData.irpf.toFixed(2)}â‚¬\n\n` +
                    `Saludos.`);
            ...
            function populateAdminTicketClients(filter = "") {
                ...
                // Agrupar por Email (o ID si no hay Email) y priorizar el registro que tenga 'authUid' (mÃ¡s completo)
                const grouped = {};
                Object.values(userMap).forEach(u => {
            ...
            async function loadAdminTicketCompanies(uid, force = false) {
                ...
                try {
                    ...
                    optAll.value = 'ALL';
                    optAll.textContent = 'ðŸ“‚ TODAS LAS SEDES';
                    ...
                    if (snap && !snap.empty) {
                        ...
                        const opt = document.createElement('option');
                        opt.value = 'comp_main';
                        opt.textContent = 'ðŸ  Empresa Principal (Defecto)';
                        select.appendChild(opt);
                        adminCompaniesCache['comp_main'] = {
                            name: uData.name || 'Empresa Principal',
                            address: uData.senderAddress || uData.street || 'Sin direcciÃ³n',
            ...
            // --- GESTIÃ“N DE SEDES Y AGENDA (ADMIN ACTIONS) ---
            ...
            async function getClientDocAliases(uid) {
                ...
                // TambiÃ©n buscar por email si recordamos el UID original
            ...
            async function loadAdminDestinationsList(uid) {
                ...
                try {
                    ...
                    tbody.innerHTML = '';
                    if (!snap || snap.empty) {
                        tbody.innerHTML = '<tr><td colspan="2" style="text-align:center; padding:20px; color:#888;">La agenda estÃ¡ vacÃ­a.</td></tr>';
                        return;
                    }
                    snap.forEach(doc => {
                        const d = doc.data();
                        ...
                        <td>
                            ...
                            <div style="font-size:0.75rem; color:#888;">${addr.address || 'Sin direcciÃ³n'}</div>
                        </td>
                        <td style="text-align:right;">
                            <div style="display:flex; gap:5px; justify-content:flex-end;">
                                <button class="btn btn-primary btn-sm" onclick="editAdminDestination('${doc.id}')" title="Editar Contacto">âœï¸</button>
                                <button class="btn btn-danger btn-sm" onclick="deleteAdminDestination('${doc.id}')" title="Eliminar Contacto">ðŸ—‘ï¸</button>
                            </div>
                        </td>
                    `;
                        tbody.appendChild(tr);
            ...
            window.deleteAdminDestination = async (destId) => {
                if (!confirm("Â¿Eliminar este contacto de la agenda?")) return;
                showLoading();
                try {
                    const aliases = await getClientDocAliases(currentAdminTargetUid);
                    for (const id of aliases) {
            ...
            function refreshAdminSuggestedSizes(uid, compId) {
                ...
                const baseSizes = ["Sobre", "PequeÃ±o", "Mediano", "Grande", "Palet", "Bulto", "Caja"];
                let items = [...baseSizes];
            ...
            document.getElementById('admin-ticket-company-select').onchange = (e) => {
                ...
                if (comp) {
                    infoText.textContent = `${comp.name} - ${comp.address || 'Sin direcciÃ³n'}`;
            ...
            async function loadAdminTicketList(direction = 'first') {
                ...
                try {
                    ...
                    // BÃºsqueda "Universal": combinamos variantes de ID numÃ©rico e Identidades (UID/Email)
                    ...
                    // AÃ±adimos tambiÃ©n los identificadores de cuenta (UID, Email) por si se guardaron ahÃ­
                    ...
                    // Limitamos a 10 elementos por restricciÃ³n de Firestore 'in'
                    ...
                    const executeSearch = (searchField, searchValues) => {
                        ...
                        adminTicketsListener = q.onSnapshot(snapshot => {
                            ...
                            if (adminCompanyTickets.length === 0) {
                                tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; padding:20px; color:#888;">${adminTicketsPage > 1 ? 'No hay mÃ¡s albaranes en esta pÃ¡gina.' : 'Todos los albaranes estÃ¡n facturados.'}</td></tr>`;
                                ...
                                <td>${pkgs}</td>
                                <td>${price.toFixed(2)}â‚¬</td>
                                <td><span class="badge ${isPrinted ? 'badge-success' : 'badge-warning'}">${isPrinted ? 'ðŸ–¨ï¸ IMPRESO' : 'ðŸ•’ NUEVO'}</span></td>
                                <td style="text-align:right;">
                                    <button class="btn btn-xs btn-outline" onclick="printTicketFromAdmin('${adminTicketUID}', '${adminTicketCompID}', '${t.docId}')">ðŸ–¨ï¸</button>
                                </td>
                            `;
                                    tbody.appendChild(tr);
                                });
                                ...
                                paginationUI.style.display = 'flex';
                                document.getElementById('label-admin-tickets-page').textContent = `PÃ¡gina ${adminTicketsPage}`;
            ...
            window.updateAdminSelectedTotal = () => {
                ...
                document.getElementById('admin-selected-total').textContent = total.toFixed(2) + 'â‚¬';
            };
            ...
            window.exportSelectedToGesco = () => {
                const checks = document.querySelectorAll('.admin-ticket-check:checked');
                if (checks.length === 0) { alert("âš ï¸ Selecciona primero los albaranes a exportar."); return; }
            ...
            window.printBulkLabelsQR = async () => {
                const checks = document.querySelectorAll('.admin-ticket-check:checked');
                if (checks.length === 0) { alert("âš ï¸ Selecciona los albaranes que deseas imprimir."); return; }
                        ...
                        <div class="label-page">
                            ...
                            <div style="display:grid; grid-template-columns: 1.5fr 1fr; gap:10px; margin-top:10px;">
                                <div>
                                    ...
                                    <p style="font-size:0.85rem;">ðŸ“ž ${t.phone || '-'}</p>
                                </div>
                            ...
                            <div style="margin-top:auto; border-top:1px solid #000; padding-top:10px; display:flex; justify-content:space-between; align-items:center;">
                                <div>
                                    ...
                                    <p style="font-size:1.1rem; font-weight:bold;">${t.shippingType || 'PAGADOS'} ${t.cod > 0 ? '| REEM: ' + t.cod.toFixed(2) + 'â‚¬' : ''}</p>
                                </div>
                                <div style="text-align:right;">
                                    <div style="border:2px solid black; padding:5px; font-weight:900; font-size:1.4rem;">${t.timeSlot || 'MAÃ‘ANA'}</div>
                                </div>
                            </div>
                        </div>
                    `);
            ...
            window.invoiceSelectedAdminTickets = async () => {
                const checks = document.querySelectorAll('.admin-ticket-check:checked');
                if (checks.length === 0) { alert("âš ï¸ Selecciona al menos un albarÃ¡n."); return; }

                if (!invCompanyData || !invCompanyData.cif) {
                    alert("âš ï¸ Configura los Datos Fiscales en la secciÃ³n FacturaciÃ³n.");
                    return;
                }
                ...
                if (subtotal <= 0) { alert("âš ï¸ Los albaranes seleccionados no tienen precio asignado."); return; }
                ...
                if (!confirm(`Â¿Generar factura por ${total.toFixed(2)}â‚¬ (${selectedTickets.length} albaranes)?`)) return;
            ...
            function renderAdminPackages() {
                ...
                <div style="display:flex; justify-content:space-between; margin-bottom:5px; background:rgba(255,255,255,0.05); padding:8px; border-radius:6px; border:1px solid var(--border-glass);">
                    ...
                    <button type="button" class="btn btn-xs btn-outline" style="border:none; color:#FF4444;" onclick="removeAdminPkg(${i})">ðŸ—‘ï¸</button>
                </div>
            `).join('');
            }
            ...
            document.getElementById('admin-manual-ticket-form').onsubmit = async (e) => {
                ...
                if (!adminTicketUID || !adminTicketCompID || adminTicketCompID === "ALL") {
                    alert("Por favor, selecciona una Sede/Empresa especÃ­fica para el remitente.");
                    return;
                }
                if (adminManPackages.length === 0) { alert("AÃ±ade al menos un bulto."); return; }

                showLoading();
                try {
                    const uData = userMap[adminTicketUID] || {};
                    const myIdNum = String(uData.idNum || '').trim();
                    if (!myIdNum) throw new Error("El cliente no tiene un nÃºmero de ID asignado.");

                    const compData = adminCompaniesCache[adminTicketCompID] || {};

                    // Generar ID correlativo buscando el mÃ¡s alto actual para esta empresa
            ...
            function resetAdminTicketForm() {
                ...
                if (timeslot) {
                    ...
                    timeslot.value = (hour >= 8 && hour < 15) ? 'MAÃ‘ANA' : 'TARDE';
                }
            ...
            async function showScannedTicket(id) {
                ...
                if (qrData) {
                    ...
                    badge.textContent = 'ðŸ“¦ PROCESANDO...';
        ...
        <div style="margin-top:10px; border-top:1px solid #EEE; padding-top:10px;">
            ...
            <b>DirecciÃ³n:</b> ${qrData.a || '---'}<br>
            <b>Bultos:</b> ${qrData.k || 1}<br>
            <b>Cod/Reemb:</b> ${qrData.c || 0}â‚¬<br>
            <i style="font-size:0.8rem; color:#666;">Cargando datos extendidos de la nube...</i>
        </div>
        `;
                }

                // Inteligencia de bÃºsqueda: Probar por DocID (completo) y luego por BusinessID (corto)
                ...
                if (doc.exists) {
                    ...
                    if (snap.empty) {
                        alert("AlbarÃ¡n no encontrado: " + searchId);
                        return;
                    }
                    doc = snap.docs[0];
                    t = doc.data();
                ...
                const badge = document.getElementById('scanned-status-badge');
                badge.textContent = isDelivered ? 'âœ… ENTREGADO' : 'âšª PENDIENTE';
        ...
        <div style="margin-top:10px; border-top:1px solid #EEE; padding-top:10px;">
            ...
            <b>DirecciÃ³n:</b> ${t.address || t.street || '---'}<br>
            <b>Bultos:</b> ${t.packagesList ? t.packagesList.reduce((s, p) => s + (parseInt(p.qty) || 1), 0) :
                        (t.packages || 1)}<br>
            <b>Estado Cloud:</b> ${isDelivered ? 'Entregado' : 'Pendiente/En TrÃ¡nsito'}
        </div>
        `;

                document.getElementById('btn-mark-scanned').onclick = async () => {
                    if (isDelivered) {
                        alert("Este albarÃ¡n ya consta como entregado.");
                        return;
                    }
                    showLoading();
                    try {
                        ...
                        alert("AlbarÃ¡n " + (t.id || id) + " marcado como ENTREGADO con Ã©xito.");
            ...
            window.openGescoImport = () => {
                if (!adminTicketUID) {
                    alert("âš ï¸ Error: No has seleccionado ningÃºn CLIENTE.");
                    return;
                }
                if (!adminTicketCompID) {
                    alert("âš ï¸ Error: No has seleccionado ninguna SEDE/EMPRESA.");
                    return;
                }
                ...
                reader.onload = async (ev) => {
                    try {
                        ...
                        if (importedTickets.length === 0) {
                            alert("âš ï¸ No se encontraron registros vÃ¡lidos en el archivo.");
                            return;
                        }

                        const targetUserName = userMap[adminTicketUID]?.name || adminTicketUID;
                        const msg = `âš ï¸ ESTÃS IMPORTANDO PARA: "${targetUserName}"\n\n` +
                            ...
                            `Â¿Confirmas la importaciÃ³n masiva?`;
                        ...
                        hideLoading();
                        alert("âœ… ImportaciÃ³n de Gesco completada.");
            ...
            async function handleImportJSON(e) {
                ...
                reader.onload = async (ev) => {
                    try {
                        ...
                        if (!confirm(`Â¿Restaurar ${ticketsToImport.length} albaranes desde el backup JSON?`)) return;
                        ...
                        for (let i = 0; i < ticketsToImport.length; i += 50) {
                            const chunk = ticketsToImport.slice(i, i + 50); const
                                batch = db.batch(); chunk.forEach(t => {
                                    ...
                                    // Asegurar clientIdNum para sincronizaciÃ³n
                        ...
                        hideLoading();
                        alert("âœ… RestauraciÃ³n de backup JSON completada.");
                        location.reload();
                    } catch (err) {
                        hideLoading();
            ...
            if (btnAdminAddPkg) {
                btnAdminAddPkg.onclick = () => {
                    ...
                    if (!size) { alert("âš ï¸ Indica el artÃ­culo/tamaÃ±o del bulto."); return; }
            ...
            function renderAdminPackagesList() {
                ...
                adminManPackages.forEach((p, i) => {
                    ...
                    <button type="button" class="btn btn-sm btn-outline" style="border:none; color:#FF4444; padding:2px 5px;" onclick="removeAdminManPkg(${i})">ðŸ—‘ï¸</button>
                `;
            ...
            async function getNextId() {
                ...
                // Variantes de ID para bÃºsqueda exhaustiva
            ...
            window.reindexUserIds = async () => {
                const p1 = confirm("âš ï¸ ATENCIÃ“N: Esta acciÃ³n reasignarÃ¡ nÃºmeros de ID correlativos (101, 102...) a todos los clientes y actualizarÃ¡ sus albaranes.\n\nÂ¿Deseas continuar?");
                if (!p1) return;
                ...
                try {
                    ...
                    alert("âœ… Clientes re-indexados con Ã©xito.");
                    loadUsers();
                } catch (e) {
